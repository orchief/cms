...
export default {
  ...
  methods: {
      getTemplate(){
        var that = this;
        this.apiGet('/admin/ExpressTemplate/'+this.$route.query.id, {}).then(res => {
            this.handelResponse(res, data => {
            this.form.freeShip=data.freeShip;
            this.form.munit = data.munit;
            this.form.name=data.name;
            this.form.specifyAreaGroups = data.specifyAreaGroups;
            data.specifyAreaGroups.map(spe=>{
                if(spe.default==1){
                    if(spe.modeId==1){
                        this.form.KD=spe;
                        this.specifyKD=true;
                    }else if(spe.modeId==2){
                        this.form.EMS=spe;
                        this.specifyEMS=true;
                    }else if(spe.modeId==3){
