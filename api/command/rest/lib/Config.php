...
class Config extends Common implements Tpl
{
    ...
    public function _init()
    {
        ...
        // 操作router生成路由
        $path = ROOT_PATH . '../cms/src/router/routes.js';
...
${1}';
    ...
    {
        $filename = ROOT_PATH . '../cms/tpl/components';
        $dest = $this->getPath() . '/components';
        copydir($filename, $dest);

        $source = ROOT_PATH . '../cms/tpl/index.vue';
    ...
    {
        $res = ROOT_PATH . '../cms/src/views/' . $this->input->getOption('vue') . '/config.js';
        return $res;
    }
    ...
    {
        $res = ROOT_PATH . '../cms/src/views/' . $this->input->getOption('vue');
        return $res;
    }
}
