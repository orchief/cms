<template>
  <div>
    ...
    <el-table :data="data.received.table" stripe="stripe" :border='true' v-loading="loading_min" element-loading-text="拼命加载中" style="width: 100%" @sort-change="sortChange">
      ...
      <el-table-column label="操作">
        <template scope="scope">
          ...
          <span class="icon_button">
            <a href="javascript:;" @click="confirmDelete(scope.row)" title="禁用">
    ...
    <!-- 编辑 -->
    <!-- 新增 -->
    <el-dialog title="编辑优惠券" :visible.sync="Edit.dialogFormVisible" @close='Edit.dialogFormVisible = false'>
...
export default {
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.productsCount = row.products.length;
      this.Edit.dialogFormVisible = true;
    },
    commitEdit(row) {
      this.Edit.data.form.products = this.Add.data.form.products
      this.$refs.form.validate(pass => {
        if (pass) {
          this.apiPut(
            this.config.baseApi + "/",
