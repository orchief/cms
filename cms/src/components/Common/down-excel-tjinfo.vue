<template>
  <div class="fl" style="margin-left: 15px">
    ...
    <el-button style='margin-bottom:20px;' type="primary" icon="document" @click="tjinfo" :loading="downloadLoading_info" v-if="tableData">导出体检信息</el-button>
  </div>
</template>
  ...
  methods: {
    ...
    infoDown(data_data){
      ...
      require.ensure([], () => {
        const { export_json_to_excel } = require("vendor/Export2Excel");
        // 导出表格配置信息
        const tHeader = this.config.titlelist;
        const filterVal = this.config.infolist;
        const fileName = this.config.title;
        const list = data_data;
        const data = this.formatJson(filterVal, list);
        that.downloadLoading_info = false;
        export_json_to_excel(tHeader, data, fileName);
      });
    },
    allPageData() {
    ...
    tjinfo(){
      this.downloadLoading_info = true;
      let url = "home/tjInfo";
      this.apiPost(url).then(res => {
      this.handelResponse(res, data => {
            console.log(res);
            this.infoDown(data);
            this.downloadLoading_info = false;
