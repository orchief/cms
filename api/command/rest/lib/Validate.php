    ...
    public function _init()
    {
        ...
        foreach($this->tableInfo as $k => $v){
            ...
            if($v['DATA_TYPE'] == 'tinyint') {
                $isMatched = preg_match_all('/(\\d+)\:/isU', $v['COLUMN_COMMENT'], $matches);
                if($isMatched){
                    $selets = implode(',', $matches[1]);
                    $sence_create[] = "in:" . $selets . $v['CHARACTER_MAXIMUM_LENGTH'];
                }
            }
        ...
        foreach($this->tableInfo as $k => $v){
            ...
            if($v['DATA_TYPE'] == 'tinyint') {
                ...
                if($isMatched){
                    $selets = implode(',', $matches[1]);
                    $sence_update[] = "in:" . $selets . $v['CHARACTER_MAXIMUM_LENGTH'];
                }
            }
