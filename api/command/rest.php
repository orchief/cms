...
class rest extends Command
{
    ...
    {
        // 名称
        $this->setName('rest')->setDescription('create restFul API demo：php think rest shop/Members -d会员');     //

        // 参数
        $this->addArgument('name',  Argument::REQUIRED, 'moudle/controller');                                   // 模块名称/控制器名称 
        $this->addArgument('omit',  Argument::OPTIONAL, 'm->model c->controller r->router v->validate', 'mcrvg');                                   // m->model c->controller r->router v->validate
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        Config::load(ROOT_PATH . '.\\config\\database.php', 'database');
