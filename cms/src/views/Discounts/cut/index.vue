<template>
    <div> 
       <div>好友砍价商品不同时享受满减活动,不能使用优惠劵,不能使用积分抵扣订单金额</div>
            ...
            <el-form ref="form" :model="Add.data.form" label-width="130px">
                ...
                <el-form-item label="砍价方式" prop="bargainType">
                    <el-radio v-model="Add.data.form.bargainType" label="1">固定值</el-radio>
                     每　　次
                    <el-input :disabled="Add.data.form.bargainType==2" v-model.trim="Add.data.form.bargainPrice" class="h-40 w-110" :maxlength="255"></el-input>
                    <br>
                    <el-radio v-model="Add.data.form.bargainType" label="2">随　机</el-radio>
                   随机砍掉
                    <el-input :disabled="Add.data.form.bargainType==1" v-model.trim="Add.data.form.bargainMinPrice" class="h-40 w-110" :maxlength="255"></el-input>-
                    <el-input :disabled="Add.data.form.bargainType==1" v-model.trim="Add.data.form.bargainMaxPrice" class="h-40 w-110" :maxlength="255"></el-input>
                </el-form-item>
        ...
        <!-- 编辑 -->
        <!-- 新增 -->
        <el-dialog title="编辑" :visible.sync="Edit.dialogFormVisible" @close='Edit.dialogFormVisible = false'>
            <el-form ref="form" :model="Edit.data.form" label-width="130px">
                ...
                <el-form-item label="砍价方式" prop="bargainType">
                    <el-radio v-model="Edit.data.form.bargainType" :label="1">固定值</el-radio>
                     每　　次
                    <el-input :disabled="Edit.data.form.bargainType==2" v-model.trim="Edit.data.form.bargainPrice" class="h-40 w-110" :maxlength="255"></el-input>
                    <br>
                    <el-radio v-model="Edit.data.form.bargainType" :label="2">随　机</el-radio>
                   随机砍掉
                    <el-input :disabled="Edit.data.form.bargainType==1" v-model.trim="Edit.data.form.bargainMinPrice" class="h-40 w-110" :maxlength="255"></el-input>-
                    <el-input :disabled="Edit.data.form.bargainType==1" v-model.trim="Edit.data.form.bargainMaxPrice" class="h-40 w-110" :maxlength="255"></el-input>
                </el-form-item>
...
export default {
  ...
  computed: {
    addbargainType() {
　　　　return this.Add.data.form.bargainType
　　},
    editbargainType() {
　　　　return this.Edit.data.form.bargainType
　　},
  },
  watch:{
    addbargainType(newValue, oldValue) {
　　　　if(newValue==1){
            this.Add.data.form.bargainMinPrice = ''
            this.Add.data.form.bargainMaxPrice = ''
        }else if(newValue==2){
            this.Add.data.form.bargainPrice = ''
        }
        console.log(this.Add.data.form)
　　},
    editbargainType(newValue, oldValue) {
　　　　if(newValue==1){
            this.Edit.data.form.bargainMinPrice = ''
            this.Edit.data.form.bargainMaxPrice = ''
        }else if(newValue==2){
            this.Edit.data.form.bargainPrice = ''
        }
　　}
  },
  methods: {
    ...
    add() {
      ...
      for(var i in this.Add.data.form){
    ...
    edit(row) {
      ...
      if(this.Edit.data.form.bargainType==1){
        this.Edit.data.form.bargainMinPrice='';
        this.Edit.data.form.bargainMaxPrice='';
      }else if(this.Edit.data.form.bargainType==2){
        this.Edit.data.form.bargainPrice='';
      }
      this.Edit.dialogFormVisible = true;
    },
    commitEdit(row) {
      console.log('his.Edit.data.form', this.Edit.data.form)
      this.$refs.form.validate(pass => {
        if (pass) {
          this.apiPut(
            this.config.baseApi + "/",
